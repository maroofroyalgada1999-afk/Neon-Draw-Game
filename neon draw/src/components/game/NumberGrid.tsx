...
export const NumberGrid: React.FC = () => {
  ...
  const numbers: GameNumber[] = useMemo(() => {
    const fallbackNumbers: GameNumber[] = Array.from({ length: 20 }, (_, i) => ({
      number: String(i + 1).padStart(2, '0'),
      multiplier: 2,
      isHighMultiplier: false,
    }));

    if (!currentRound || !currentRound.numbers || !Array.isArray(currentRound.numbers)) {
      return fallbackNumbers;
    }

    const roundNumberMap = new Map<string, GameNumber>();
    currentRound.numbers.forEach((n) => {
      if (n && typeof n.number === 'string') {
        roundNumberMap.set(n.number, n);
      }
    });

    return fallbackNumbers.map((fb) => {
      const fromRound = roundNumberMap.get(fb.number);
      return fromRound || fb;
    });
  }, [currentRound]);
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-mono font-black border ${
                  totalDistinct >= 20
