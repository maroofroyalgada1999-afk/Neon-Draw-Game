...
import { Round, Bet, GameConfig, RoundPhase, PreviousRoundResult, isValid20NumberRound, VALID_GAME_NUMBERS_SET } from '../../src/types/index.js';
...
export class RoundManager {
  ...
  public ensureActiveRound(): Round {
    ...
    if (currentId) {
      ...
      if (existing && isValid20NumberRound(existing) && existing.status !== 'SETTLED' && existing.status !== 'CANCELLED') {
        const now = Date.now();
  ...
  public createNewRound(): Round {
    ...
    const lastSettledRound = allRounds.find((r) => r.status === 'SETTLED' && isValid20NumberRound(r)) || allRounds[0];
    ...
    // Authoritative 20 tiles generation with newly randomized positions:
    // 14 normal (2x-10x) and 6 high (11x-20x)
    const numbers = CryptoRNG.generateMultipliers(
      ...
      config.normalMultiplierCount || 14,
      config.highMultiplierCount || 6
    );
    ...
    if (lastSettledRound && lastSettledRound.winningNumber && VALID_GAME_NUMBERS_SET.has(lastSettledRound.winningNumber)) {
