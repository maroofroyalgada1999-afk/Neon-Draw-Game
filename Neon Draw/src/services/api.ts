...
class ApiService {
  ...
  private getLocalFallbackRound() {
    ...
    // Deterministic 80/20 multiplier grid with dynamic seed
    const roundIdx = Math.floor(now / cycle);
    const numbers = Array.from({ length: 100 }, (_, i) => {
      const numStr = i.toString().padStart(2, '0');
      // Rotate 20 high multipliers dynamically per round
      const isHigh = (i + roundIdx * 17) % 5 === 0;
      const multiplier = isHigh ? 11 + ((i + roundIdx) % 10) : 2 + ((i + roundIdx) % 9);
      return {
        number: numStr,
        multiplier,
        isHighMultiplier: isHigh,
      };
    });
